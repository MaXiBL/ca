...
import { VERIFY_PWD } from 'service'
...
export default class LayerDelCase extends Vue {
  $refs!: {
    vue: Vue,
    vues: Vue[],
    element: HTMLElement,
    ...
    if (!this.userPwdWarning) {
      VERIFY_PWD({
        ...
        if (res.data.success) {
          ...
          this.userPwdWarning = true
          this.userPwdWarningText = res.data.message
        }
      })
    }
  }
